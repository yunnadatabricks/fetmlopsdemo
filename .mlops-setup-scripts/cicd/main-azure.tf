resource "databricks_group" "mlops-service-principal-group-staging" {
  display_name = "fetmlopsdemo-service-principals"
...
module "azure_create_sp" {
  ...
  service_principal_name       = "fetmlopsdemo-cicd"
  project_directory_path       = "/fetmlopsdemo"
  azure_tenant_id              = var.azure_tenant_id
  service_principal_group_name = "fetmlopsdemo-service-principals"
}
...
provider "databricks" {
  alias = "staging_sp"
  host  = "https://adb-2778482320628131.11.azuredatabricks.net"
  token = module.azure_create_sp.staging_service_principal_aad_token
}

provider "databricks" {
  alias = "prod_sp"
  host  = "https://adb-8783378989168355.15.azuredatabricks.net"
