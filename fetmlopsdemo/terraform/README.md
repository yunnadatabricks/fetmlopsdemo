...
This directory (`fetmlopsdemo/terraform`) includes Databricks ML resource configuration, i.e. the definitions of a batch
...
the contents of `fetmlopsdemo/terraform` to the Git repo:
```
git checkout -b add-ml-resource-config
git add fetmlopsdemo/terraform
...
Then, resolve the TODOs in `fetmlopsdemo/terraform/staging/inference-job.tf` and `fetmlopsdemo/terraform/prod/inference-job.tf`, passing
...
To get started, open `fetmlopsdemo/terraform/staging/inference-job.tf`.  The file contains the ML resource definition of
a batch inference job, like:

```$xslt
resource "databricks_job" "batch_inference_job" {
  name = "${local.env_prefix}fetmlopsdemo-batch-inference-job"
  ...
  notebook_task {
    notebook_path = "fetmlopsdemo/deployment/batch_inference/notebooks/BatchInference"
  }
  ...
}
```

The example above defines a Databricks job with name `staging-fetmlopsdemo-batch-inference-job`
that runs the notebook under `fetmlopsdemo/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model
for batch inference. 
